...
- **PR #476, MERGED** (`feat/adaptive-verify-master-password`) — verifies the
  typed master password before deriving the adaptive fingerprint key, closing
  a gap flagged 3x across this PR's own review rounds (Codex 1-2, CodeRabbit
  round 3) and deferred each time for want of a verification primitive. Found
  one on the standard login path: `sessionVaultCryptoV3.unlockWithMasterPassword`
  already unwraps the same secret via AES-GCM, which fails loudly on the wrong
  KEK. Constraint that shaped the design: the wrapped-DEK endpoint is
  throttled at 3/hour/user and login already spends one, so verification had
  to cost zero network calls — solved by caching the wrapped envelope
  (non-secret ciphertext) and adding a verify-only re-attempt that discards
  its result. **Round-1 review-fix** (CodeRabbit, commit `40c6110`): added the
  one proposed Trivial test (pins `changeMasterPassword`'s "refresh the cache
  only after the PUT succeeds" ordering on a failed PUT, mutation-checked);
  declined the same finding's twin ask for `rewrapMasterPasswordFromRecovery`
...
- **PR #477, MERGED** (`feat/adaptive-delta-aware-memorability-weights`) —
  ...
  out-of-scope. See §4.6's "Superseded" note for the full mechanism change
  and its own round-1 review-fix entry.
- **PR #478, OPEN** (`fix/vault-v3-unlock-degraded-mode`) — surfaces the v3
  ...
  recovery flow via a sign-out-first CTA. **Round-1 review-fix** (CodeRabbit):
  `vaultV3Degraded` was only ever set `true`, never reset — fixed to clear on
  a clean re-unlock and on logout, with the stable toast now dismissed on that
  transition. **Round-2** (CI infrastructure, not a review-bot finding): two
  genuinely-failing checks (`build-and-sbom`, `Codebase Security Scan (FS)`)
  both traced to a transient GitHub releases-API 503 hitting a third-party
  tool's install step with no resilience, unlike a sibling step in the SAME
...
low-blast-radius changes. See [[epigenetic-adaptation-phases]] (memory) for
full narrative detail on each.
