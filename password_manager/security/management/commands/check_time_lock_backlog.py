    ...
    def handle(self, *args, **options):
        ...
        for will in candidates:
            if will.trigger_type == 'inactivity' and will.inactivity_days is not None:
                deadline = will.last_check_in + timezone.timedelta(days=will.inactivity_days)
                if now >= deadline:
        ...
        for will, deadline in overdue_wills:
            ...
            self.stdout.write(
                f'  PasswordWill {will.id} -- owner={will.owner.username!r} '
                f'trigger_type={will.trigger_type!r} '
                f'overdue_by={overdue_by} '
                f'capsule={will.capsule.title!r}'
            )
        ...
        for escrow in overdue_escrows:
            ...
            self.stdout.write(
                f'  EscrowAgreement {escrow.id} -- title={escrow.title!r} '
