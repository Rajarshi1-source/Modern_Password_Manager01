...
class ForceRotationEndpointTests(TestCase):
    ...
    def test_rotate_records_event_with_reason_only(self):
        ...
        resp = self.client.post(
            self.url, {"reason": "Proactive rotation"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_202_ACCEPTED)

        event = PasswordRotationEvent.objects.get(
...
class CompleteRotationEndpointTests(TestCase):
    ...
    def test_complete_marks_pending_event_completed(self):
        ...
        resp = self.client.post(
            self.complete_url, {"event_id": event_id}, format="json"
        )
    ...
    def test_complete_targets_the_specified_event_only(self):
        ...
        first_rotate = self.client.post(
            self.rotate_url, {"reason": "first"}, format="json"
        )
        second_rotate = self.client.post(
            self.rotate_url, {"reason": "second"}, format="json"
        )
        ...
        self.assertEqual(
            PasswordRotationEvent.objects.get(event_id=first).outcome, "completed"
        )
        self.assertEqual(
            PasswordRotationEvent.objects.get(event_id=second).outcome, "pending"
        )
    ...
    def test_complete_is_idempotent(self):
        ...
        self.assertEqual(
            PasswordRotationEvent.objects.filter(
                event_id=event_id, outcome="completed"
            ).count(),
            1,
        )
