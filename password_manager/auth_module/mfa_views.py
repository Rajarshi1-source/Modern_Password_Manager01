...
from .biometric_processing import decode_face_image, decode_voice_audio
...
def update_continuous_auth(request):
    ...
    try:
        ...
        if request.data.get('face_image'):
            face_data = base64.b64decode(request.data['face_image'])
            face_image = decode_face_image(face_data)
            session.face_checks_count += 1
        
        if request.data.get('voice_audio'):
            voice_data = base64.b64decode(request.data['voice_audio'])
            voice_features = decode_voice_audio(voice_data)
